...
export function PlansPage({plans}:{plans:Plan[]}){
 ...
 return <>
  ...
  <section className="plan-grid">{visible.map(plan=>{const progress=plan.task_count?Math.round(plan.completed_count/plan.task_count*100):0;return <button className="plan-card" key={plan.id} onClick={()=>navigate(`/plans/${plan.id}`)}><span className={`badge ${plan.priority}`}>{priorityLabel[plan.priority]}</span><h3>{plan.title}</h3><span className="plan-card-stats"><b>할 일 {plan.task_count}개 · {plan.completed_count}개 완료</b><small>최종 마감일 <b>{plan.final_due_date??'없음'}</b></small></span><span className="plan-progress" role="progressbar" aria-label={`${plan.title} 진행률`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}><i aria-hidden="true" style={{width:`${100-progress}%`}}/></span><small className="plan-progress-label"><b>{progress}%</b> 진행</small></button>})}{!visible.length&&<div className="panel empty">{plans.length?'조건에 맞는 계획이 없습니다.':'저장된 계획이 없습니다. 첫 계획을 만들어 주세요.'}</div>}</section>
 </>
}
