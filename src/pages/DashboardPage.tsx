...
import {createdDateLabel,minutes,today} from '../lib/types'
...
export function DashboardPage({plans}:{plans:Plan[]}){
 ...
 const byDate=useMemo(()=>{const grouped=new Map<string,Task[]>();for(const task of taskRows)grouped.set(task.due_date,[...(grouped.get(task.due_date)??[]),task]);for(const rows of grouped.values())rows.sort((a,b)=>Number(a.status==='completed')-Number(b.status==='completed'));return grouped},[taskRows])
 ...
 const selectedTasks=useMemo(()=>[...(byDate.get(selectedDate)??[])].sort((a,b)=>Number(a.status==='completed')-Number(b.status==='completed')),[byDate,selectedDate])
 const selectedCompleted=selectedTasks.filter(task=>task.status==='completed').length
 const selectedRemainingMinutes=selectedTasks.filter(task=>task.status!=='completed').reduce((sum,task)=>sum+Math.max(task.estimated_minutes-task.actual_minutes,0),0)
 const selectedDateLabel=new Intl.DateTimeFormat('ko-KR',{timeZone:'Asia/Seoul',month:'long',day:'numeric',weekday:'long'}).format(new Date(`${selectedDate}T00:00:00+09:00`))
 ...
 return <>
  ...
  <aside className="panel day-agenda"><div className="agenda-head"><div className="eyebrow">선택한 날짜</div><div className="agenda-date-row"><h2>{selectedDateLabel}</h2>{selectedDate===today&&<span>오늘</span>}</div><div className="agenda-summary">{tasks===null?<span>일정을 불러오는 중입니다.</span>:selectedTasks.length?<><span><b>{selectedTasks.length}개</b> 중 <b>{selectedCompleted}개 완료</b></span><span>남은 예상 <b>{minutes(selectedRemainingMinutes)}</b></span></>:<span>등록된 일정이 없습니다.</span>}</div></div><div className="agenda-list">{tasks===null?<div className="empty loading-state">일정을 불러오는 중입니다.</div>:<>{selectedTasks.map(task=><button className="agenda-item" onClick={()=>navigate(`/plans/${task.plan_id}/tasks/${task.id}/action`)} aria-label={`${task.title} 상세보기`} key={task.id}><span className="agenda-item-head"><span className={`agenda-status ${task.status}`}>{task.status==='completed'?'완료':'진행 중'}</span><span className="agenda-detail-label">상세보기</span></span><strong className="agenda-item-title">{task.title}</strong><span className="agenda-item-meta"><span>{task.plan_title}, 예상 {minutes(task.estimated_minutes)}</span><small>작성 {createdDateLabel(task.created_at)}</small><span className="task-tags-inline">{task.tags.map(tag=><i key={tag}>#{tag}</i>)}</span></span></button>)}{!selectedTasks.length&&<div className="empty">이 날짜에 마감할 일이 없습니다.</div>}</>}</div><button className="primary create-button create-entry" onClick={()=>navigate(`/plans/new?dueDate=${encodeURIComponent(selectedDate)}`)}>새 계획 만들기</button></aside>
  </div>
 </>
}
