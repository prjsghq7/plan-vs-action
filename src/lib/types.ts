...
export type Plan={id:string;title:string;priority:Priority;success_criteria:string;task_estimated_minutes:number;task_count:number;completed_count:number;final_due_date:string|null;created_at:string;updated_at:string}
export type Task={id:string;plan_id:string;plan_title:string;title:string;due_date:string;priority:Priority;estimated_minutes:number;actual_minutes:number;status:'active'|'completed';status_version:number;tags:string[];created_at:string;updated_at:string}
...
export const createdDateLabel=(value:string)=>new Intl.DateTimeFormat('ko-KR',{timeZone:'Asia/Seoul',year:'numeric',month:'numeric',day:'numeric'}).format(new Date(value))
